...
x = [100, 150, 100]; y = [100, 100, 150]; radius = 5; color = (0, 0, 0); vel_x = 0; vel_y = 0; mass = 0.1
objects = [CircleObject(i, x[i], y[i], radius, color, vel_x, vel_y, mass) for i in range(len(x))]
xS = WIDTH//2; yS = HEIGHT//2; radiusS = 20; colorS = (255, 0, 0); mass = 0.1
...
while True:
    ...
    if dmList.count(False) == num_objects:
        ...
        static_object.RKMove(dest, 16)
        for obj in objects:
            obj.RKMove(pairsPlusVec[obj.id], 16)
            # obj.vel_x += destVel_x
            # obj.vel_y += destVel_y
            
    else:
        index = dmList.index(True)
        dmList[index] = objects[index].RKMove(pairs[index],16)
        # for obj in objects:
        #     dmList[obj.id] = obj.RKMove(pairs[obj.id], 16)
